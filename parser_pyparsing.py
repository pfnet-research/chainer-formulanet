...
ident = Word("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#@!^~?$'\_%+-*<>=/\\") | Literal(
    "..") | Literal(",")
...
expr_cont_expr1 = (expr + ((binop + expr + rparen).setParseAction(
    lambda toks: lambda e1: EApply(EApply(EIdent(toks[0]), e1), toks[1])) | (expr + rparen).setParseAction(
    lambda toks: lambda e1: EApply(e1, toks[0])))).setParseAction(lambda toks: toks[1](toks[0]))
...
thm = (Group(sep_by(expr, Literal(",")).setParseAction(lambda toks: list(toks))) + Literal(
    "|-").suppress() + expr).setParseAction(lambda toks: Thm(*toks))
