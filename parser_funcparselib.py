...
from funcparserlib.parser import Parser, some, maybe, many, skip, forward_decl
...
def tokenize(s: str) -> List[Token]:
    return [x for x in _tokenizer(s) if x.type not in _tokenizer_useless]
...
expr_cont_expr1 = (expr + (
            ((binop + expr + rparen) >> (lambda xs: lambda e1: EApply(EApply(EIdent(xs[0]), e1), xs[1]))) | (
                (expr + rparen) >> (lambda e2: lambda e1: EApply(e1, e2))))) >> (lambda xs: xs[1](xs[0]))
